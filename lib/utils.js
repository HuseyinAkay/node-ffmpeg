...
module.exports.durationToSeconds = function (duration) {
   ...
   return parseInt(parts[0], 10) * 3600 + parseInt(parts[1], 10) * 60 + parseFloat(parts[2], 10);
};
 ...
 * e.g. (input file)
 */
module.exports.addQuotes = function (fileName) {
   // Add quotes
   fileName = this.removeQuotes(fileName);
   return JSON.stringify(fileName);
}

module.exports.removeQuotes = function (fileName) {

   if (isString(fileName) && fileName.endsWith('\'') && fileName.charAt(0) == '\'')
      fileName = fileName.split('\'', 1).at(0);
   return fileName
}