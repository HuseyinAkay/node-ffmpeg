...
class ffmpeg {
   constructor( /* inputFilepath, settings, callback */) {
      ...
      var _ffmpegInfoConfiguration = function (settings) {
         ...
         utils.exec([bin, '-formats', '2>&1'], settings, function (error, stdout, stderr) {
      ...
      var _videoInfo = function (fileInput, settings) {
         ...
         var deferred = when.defer();
         // Make the call to retrieve information about the ffmpeg
         utils.exec([bin, '-i', utils.addQuotes(fileInput), '2>&1'], settings, function (error, stdout, stderr) {
