...
var errors		= require('./errors')
  ...
  , video		= require('./video')
  , config        = require('./ffmpegConfig');

class ffmpeg {
   constructor( /* inputFilepath, settings, callback */) {
      ...
      var _ffmpegInfoConfiguration = function (settings) {
         ...
         utils.exec([config.bin, '-formats', '2>&1'], settings, function (error, stdout, stderr) {
            ...
            for (var i in codecList) {
               ...
               if (match) {
                  ...
                  var scope = match[1].replace(/\s/g, ''), extension = match[2].replace(/\s/g, '');
